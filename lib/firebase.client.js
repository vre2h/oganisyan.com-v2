...
export async function getByCondition(dName, condition = {}, sorters) {
  const conditions = Object.values(condition).map((item) => {
    return where(item.key, item.condition || "==", item.value);
  });
  const q = query(collection(client.db, dName), ...conditions, ...sorters);
